...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	async def _prepare_context(self, step_info: AgentStepInfo | None = None) -> BrowserStateSummary:
		...
		browser_state_summary = await self.browser_session.get_state_summary_with_fallback(cache_clickable_elements_hashes=True)
	...
	async def multi_act(
		...
		for i, action in enumerate(actions):
			...
			if action.get_index() is not None and i != 0:
				new_browser_state_summary = await self.browser_session.get_state_summary_with_fallback(

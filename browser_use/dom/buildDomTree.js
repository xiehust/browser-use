(
  ...
  /**
   * Checks if an element is interactive - optimized for better accuracy and performance.
   */
  function isInteractiveElement(element) {
    ...
    // Early exit for disabled elements
    if (element.disabled || element.inert || element.readOnly) {
      return false;
    }

    // Check for explicitly disabled attributes
    ...
    // Non-interactive cursor styles indicate element shouldn't be interacted with
    ...
    // Primary interactive elements - always interactive unless disabled
    const primaryInteractiveElements = new Set([
      'a', 'button', 'input', 'select', 'textarea', 'summary', 'option'
    ]);
    if (primaryInteractiveElements.has(tagName)) {
      return true;
    }

    // Interactive cursor styles - strong signal of interactivity
    const interactiveCursors = new Set([
      'pointer', 'grab', 'grabbing', 'zoom-in', 'zoom-out', 'move'
    ]);
    ...
    const interactiveRoles = new Set([
      ...
      'combobox', 'searchbox', 'textbox', 'option'
    ]);
    if (interactiveRoles.has(role)) {
      return true;
    }

    // Common interactive class names and attributes
    ...
    // Data attributes that typically indicate interactivity
    ...
    const eventHandlerAttrs = ['onclick', 'onmousedown', 'onmouseup', 'ondblclick'];
    ...
    // Check for event listeners (when available)
    try {
      if (typeof getEventListeners === 'function') {
        const listeners = getEventListeners(element);
        const interactiveEvents = ['click', 'mousedown', 'mouseup', 'dblclick', 'submit'];
        for (const eventType of interactiveEvents) {
      ...
      // getEventListeners not available in all contexts
    }

...
class BrowserSession(BaseModel):
	...
	async def on_BrowserStartEvent(self, event: BrowserStartEvent) -> dict[str, str]:
		...
		self.logger.debug(f'[BrowserSession] Starting browser session: cdp_url={self.cdp_url}, is_local={self.is_local}')
		...
		try:
			...
			if not self.cdp_url:
				if self.is_local:
					...
					self.logger.debug('[BrowserSession] Dispatching BrowserLaunchEvent...')
					...
					try:
						...
						self.logger.debug(f'[BrowserSession] Browser launched successfully with CDP URL: {self.cdp_url}')
					except Exception as launch_error:
						self.logger.error(f'[BrowserSession] Failed to get browser launch result: {launch_error}')
			...
			if self._cdp_client_root is None:
				...
				self.logger.debug(f'[BrowserSession] Connecting to CDP at {self.cdp_url}...')
				try:
					await self.connect(cdp_url=self.cdp_url)
					self.logger.debug(
						f'[BrowserSession] CDP connection successful, _cdp_client_root={self._cdp_client_root is not None}'
					)
					...
					self.logger.error(f'[BrowserSession] CDP connection failed: {connect_error}')
					raise RuntimeError(f'Failed to connect to CDP at {self.cdp_url}: {connect_error}') from connect_error
			else:

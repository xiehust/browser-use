...
## Planner Support Discontinued

<Note>
  **Important**: Planner support has been removed as of version 0.3.2. The agent capability for planning is significantly improved and no longer requires the planner system. All planner-related parameters (`planner_llm`, `planner_interval`, `use_vision_for_planner`, `is_planner_reasoning`, `extend_planner_system_message`) are now deprecated and will be ignored.
</Note>

The agent's improved planning capabilities now handle complex multi-step tasks without requiring a separate planner model. This change:

- Simplifies the agent configuration
- Reduces complexity and potential points of failure
- Improves performance by eliminating the overhead of separate planning calls
- Maintains all the benefits of strategic task planning through enhanced agent reasoning

...
### Planner System Prompt (Deprecated)

<Note>
  **Important**: Planner support has been removed as of version 0.3.2. The `extend_planner_system_message` parameter is deprecated and will be ignored. The agent's built-in planning capabilities now handle task decomposition and strategic thinking without requiring a separate planner system.
</Note>
